#!/usr/bin/env python3
# Timestamp (UTC): 2026-01-04T22:44:30Z
"""Train SELL entry regressor (oracle15m) with ctx120 features + weighting + oracle-entry timing helper.

You clarified:
- Precontext lookback stays 5 minutes (t-5..t-1) + causal ctx series.
- Use the same target as before: y_sell[t] = net_ret_pct_sell(close[t], min(close[t+1..t+15]), fee_side).
- Add an *oracle-entry helper* in training to improve entry timing.

Oracle-entry timing helper (E=5 minutes)
- For each minute t, define best_future_5m[t] = max(y_sell[t..t+4]).
- Define timing_score[t] = exp((y_sell[t] - best_future_5m[t]) / temp).
  - timing_score is in (0,1], equals 1 when t is the best entry inside the next 5 minutes.
- Gate it to opportunity windows: score *= 1[best_future_5m[t] >= ret_thresh] (default ret_thresh=0.2).
- timing_weight[t] = 1 + gamma * timing_score[t].

Final sample_weight = ref_weight * timing_weight.

This uses future ONLY to shape training weights; inference remains purely causal.

Outputs (timestamped)
- data/entry_regressor_oracle15m_sell_ctx120_weighted_oracleentry5m_<ts>/
  - run_meta_<ts>.json
  - metrics_<ts>.json
  - feature_importance_<ts>.csv
  - models/entry_regressor_sell_oracle15m_ctx120_weighted_oracleentry5m_<ts>.joblib
"""
...
def _past_5m_features_with_ctx(
    ...
    *,
    ...
    """Build 5-minute precontext features over t-5..t-1 for base + ctx series."""
...
def _future_max_including_current(x: np.ndarray, W: int) -> np.ndarray:
    """Return max over x[t:t+W] (W>=1). NaN-safe."""
    x = np.asarray(x, dtype=np.float64)
    if W <= 1:
        return x.copy()
    # reverse + rolling max gives max over current and previous in reversed => current..future in original
    s = pd.Series(x[::-1])
    m = s.rolling(W, min_periods=1).max().to_numpy(np.float64)
    return m[::-1]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train weighted SELL entry regressor (oracle15m) with ctx120 + oracle-entry(5m) helper")
    ap.add_argument(
        "--bars",
    ...
    # oracle-entry timing helper
    ap.add_argument("--entry-window", type=int, default=5, help="Oracle entry window in minutes (include current), default 5")
    ap.add_argument("--entry-temp", type=float, default=0.2, help="Temperature in % for exp((y-best)/temp)")
    ap.add_argument("--entry-gamma", type=float, default=2.0, help="Scale for oracle-entry timing weight")
    ap.add_argument("--ret-thresh", type=float, default=0.2, help="Opportunity gate for oracle-entry helper")
    ...
    ts = now_ts()
    out_root = Path(args.out_dir) / f"entry_regressor_oracle15m_sell_ctx120_weighted_oracleentry5m_{ts}"
    ...
    avg = ssum / float(scount) if scount > 0 else np.zeros(n, dtype=np.float64)
    alpha = float(args.weight_alpha)
    w_ref = 1.0 + alpha * avg

    # --- oracle-entry timing helper ---
    entry_w = int(args.entry_window)
    temp = float(args.entry_temp)
    gamma = float(args.entry_gamma)
    ret_thresh = float(args.ret_thresh)

    best_future = _future_max_including_current(y_sell, entry_w)
    # timing_score in (0,1]
    d = (y_sell - best_future) / max(1e-9, temp)
    d = np.clip(d, -50.0, 0.0)
    timing_score = np.exp(d)
    timing_score = np.where(best_future >= ret_thresh, timing_score, 0.0)
    w_timing = 1.0 + gamma * timing_score
    ...
    print(
        f"[5/6] Training weighted SELL regressor (+oracle-entry {entry_w}m): train={len(tr_idx):,} val={len(va_idx):,} test={len(test_idx):,} n_features={len(X_names):,}",
        flush=True,
    )
    ...
    # quick win-AUC on test
    try:
        from sklearn.metrics import roc_auc_score

        y_test = y_sell[test_idx]
        p_test = pred_all[test_idx]
        win_test = (y_test >= ret_thresh).astype(int)
    ...
    metrics = {
        "SELL": {
            "auc_test_win_y>=ret_thresh": float(auc_test),
    ...
    artifact = {
        ...
        "metrics": metrics,
        "weighting": {
            "ref": {"shift_csv": str(shift_path), "shift_top_k": int(args.shift_top_k), "weight_alpha": float(alpha)},
            "oracle_entry": {"entry_window_min": int(entry_w), "temp": float(temp), "gamma": float(gamma), "ret_thresh": float(ret_thresh)},
        },
    }

    model_path = models_dir / f"entry_regressor_sell_oracle15m_ctx120_weighted_oracleentry5m_{ts}.joblib"
