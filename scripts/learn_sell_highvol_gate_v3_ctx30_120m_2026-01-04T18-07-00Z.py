#!/usr/bin/env python3
# Timestamp (UTC): 2026-01-04T18:07:00Z
"""High-vol regime split for SELL entries, adding 30–120m context features (local-only).

Goal
- Keep the existing pipeline unchanged for entry-model scoring + HV regime isolation.
- Add longer-horizon (30/60/120m) context features onto the loaded bars DataFrame *inside this script only*.
- Retrain the HV good-vs-bad gate and report whether AUC improves.

Context features added (per-minute, causal rolling / shifts)
- mom_{W}m_pct: close pct-change vs t-W
- vol_std_{W}m: rolling std on close using population variance (matches existing vol_std_5m helper style)
- range_{W}m: rolling (max(high)-min(low))
- range_norm_{W}m: range_{W}m / close
- vwap_dev_{W}m: rolling VWAP deviation over W minutes

Outputs (timestamped)
- data/highvol_sell_gate_v3_ctx_{ts}/
  - summary_{ts}.json
  ...
  - guardrail_sweep_1d_{ts}.csv (if non-empty)
  - optionally: highvol_dataset_{ts}.parquet, gate_dataset_{ts}.parquet (if --save-datasets)
...
def _rolling_sum_nan_prefix(x: np.ndarray, w: int) -> np.ndarray:
    """Rolling sum with NaN for first w-1 entries (matches crossref style)."""
    x = np.asarray(x, dtype=np.float64)
    n = x.size
...
def _rolling_std_pop_nan_prefix(x: np.ndarray, w: int) -> np.ndarray:
    """Rolling std using population variance (ddof=0), NaN for first w-1."""
    x = np.asarray(x, dtype=np.float64)
    n = x.size
    out = np.full(n, np.nan, dtype=np.float64)
    if n == 0:
...
def _add_context_features(df: pd.DataFrame, windows: list[int]) -> None:
    """Add 30/60/120m context features to df in-place."""
    close = pd.to_numeric(df["close"], errors="coerce").to_numpy(np.float64, copy=False)
    ...
    for w in windows:
        ...
        df[f"mom_{w}m_pct"] = mom

        # realized vol proxy (std of close, pop variance to match existing helper)
        df[f"vol_std_{w}m"] = _rolling_std_pop_nan_prefix(close, w)

        # range
        r = (s_high.rolling(w, min_periods=w).max() - s_low.rolling(w, min_periods=w).min()).to_numpy(np.float64)
        df[f"range_{w}m"] = r
        df[f"range_norm_{w}m"] = r / np.maximum(1e-9, close)
        ...
        df[f"vwap_dev_{w}m"] = vwap_dev


def main() -> None:
    ap = argparse.ArgumentParser(description="High-vol SELL gate v3 with 30/60/120m context features")
    ap.add_argument(
        "--bars",
    ...
    ap.add_argument(
        "--ctx-windows",
        default="30,60,120",
        help="Comma-separated context windows (minutes) to add, e.g. 30,60,120",
    )

    ap.add_argument(
        "--save-datasets",
        action="store_true",
        help="If set, save high-vol and gate datasets for offline inspection",
    )
    ...
    # Add 30/60/120m context features (local-only)
    ctx_windows = [int(x.strip()) for x in str(args.ctx_windows).split(",") if x.strip()]
    print(f"Adding context features for windows: {ctx_windows}", flush=True)
    _add_context_features(df, ctx_windows)
    ...
    # Base selected dataframe with entry score + oracle
    ...
    # Attach context features (raw per-minute values) to the selection
    ctx_cols = []
    ...
    sel_ctx = df.loc[sel_idx, ctx_cols].reset_index(drop=True)

    sel = pd.concat([sel_base, sel_feat.reset_index(drop=True), sel_ctx], axis=1)

    # Define high-vol regime within selection (same proxies as before)
    proxies = [
        "vol_std_5m__last",
        "range_norm_5m__max5",
        "vol_log1p__last",
    ...
    ts = now_ts()
    out_root = Path(args.out_dir) / f"highvol_sell_gate_v3_ctx_{ts}"
    ...
    cand = []
    if not shifts.empty:
        cand.extend(shifts.head(80)["feature"].tolist())
    cand.extend(
        [
            "ret_1m_pct__m1",
            "ret_1m_pct__m2",
    ...
    # ensure some ctx features are swept too
    cand.extend(ctx_cols)
