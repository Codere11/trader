#!/usr/bin/env python3
# Timestamp (UTC): 2026-01-08T07:47:09Z
...
# Minimum transfer amount (USDC) to avoid quantization rounding to zero
MIN_TRANSFER_USDC = 0.01
...
class EthSellSimpleRunner:
    ...
    async def _ensure_trading_floor(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"_ensure_trading_floor: failed to get bank equity: {e}")
            bank_eq = 0.0
        ...
        transfer = min(float(needed), float(bank_eq), float(budget_remaining))
        ...
        if transfer <= 0.0:
            print(f"_ensure_trading_floor: no transfer possible (needed={needed:.6f}, bank_eq={bank_eq:.2f}, budget_remaining={budget_remaining:.2f})")
            return {
                "topped_up": False,
                "needed_usdc": float(needed),
                "transferred_usdc": 0.0,
        ...
        print(f"_ensure_trading_floor: topped up {transfer:.2f} USDC (spent={self.initial_topup_spent_usdc:.2f}/{self.initial_topup_budget_usdc:.2f})")
        return {
            "topped_up": True,
            "needed_usdc": float(needed),
    ...
    async def _siphon_profit_and_topup(self, *, equity_before: float) -> Dict[str, Any]:
        try:
            ...
            print(f"_siphon_profit_and_topup: failed to get trading equity after trade: {e}")
        ...
        if profit > 0:
            ...
            if requested > 0:
                try:
                    ...
                    print(f"_siphon_profit_and_topup: failed to get bank equity: {e}")
                    bank_eq = 0.0
                
                amt_q = usdc_to_quantums(requested)
                if amt_q <= 0:
                    print(f"_siphon_profit_and_topup: siphon amount rounded to zero quantums (requested={requested:.6f})")
                    siphon = {
                        "attempted": False,
                        ...
                        "transferred_usdc": 0.0,
                        "skipped": True,
                        "skip_reason": "siphon_amount_rounded_to_zero",
                    }
                else:
                    tx = await self.clients.node.transfer(
                        self.clients.wallet,
                    ...
                    print(f"_siphon_profit_and_topup: siphoned {requested:.2f} USDC (profit={profit:.2f}, frac={frac})")
                    siphon = {
                        "attempted": True,
                        "profit_usdc": float(profit),
                        "requested_usdc": float(requested),
